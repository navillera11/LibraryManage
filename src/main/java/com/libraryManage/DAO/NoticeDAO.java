...
public class NoticeDAO {
  ...
  public List<NoticeDTO> showAll() {
    List<NoticeDTO> result = jdbcTemplate.query("SELECT * FROM NOTICE;", (rs, rowNum) -> {
      NoticeDTO noticeDTO = new NoticeDTO(rs.getInt("ID"), rs.getString("TITLE"),
          rs.getDate("DATE"),
          rs.getString("CONTENT"));
      return noticeDTO;
    });
  ...
  public List<NoticeDTO> showFive() {
    List<NoticeDTO> result = jdbcTemplate.query("SELECT * FROM NOTICE ORDER BY ID DESC LIMIT 5;",
        (rs, rowNum) -> {
          NoticeDTO noticeDTO = new NoticeDTO(rs.getInt("ID"), rs.getString("TITLE"),
              rs.getDate("DATE"),
              rs.getString("CONTENT"));
  ...
  public void insertNotice(NoticeDTO _noticeDTO) {
    ...
    jdbcTemplate.update(
        "INSERT INTO NOTICE(TITLE, DATE, CONTENT) VALUES('" + noticeDTO.getNoticeTitle()
